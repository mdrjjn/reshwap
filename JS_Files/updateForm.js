function updateForm() {
  ...
  //Beginning to update the form
  //a reference object for the form class in the database + its id value concatenated
  var formRef = firebase.database().ref("forms/" + formId);
  var newForm = {
    category: document.getElementById("Category").value,
    title: document.getElementById("Title").value,
    description: document.getElementById("Description").value,
    courseName: "not mentioned",
  ...
  if (cat == "Books") { //only if the category is books at the momment of the upload
    newForm.courseName = document.getElementById("ChooseSubject").value;
    newForm.subject = document.getElementById("SelectSubject").value;
  }

  if (document.getElementById("imageContainer")) { //the messed up if statement is coz if u update the image, then the functions on the MyForms create not an image, but canvas
    if (document.getElementById("imageContainer").name === "hasFile") {
      filePresent = true;
    } else {
      filePresent = false;
    }
  ...
  if (filePresent && fileChanged) {
    ...
    newForm.imgName = name;
    //a new reference object specifically for the image storage

    var imageRef = firebase.storage().ref().child('images/' + name);
